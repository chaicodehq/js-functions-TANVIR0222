...
export function createElection(candidates) {
  ...
  return {
    ...
    getResults(sortFn) {
      // Convert votes object to array with candidate info
      ...
      // Sort: use custom sortFn if provided
...
export function createVoteValidator(rules) {
  return function (voter) {
    if (!voter) return { valid: false, reason: "Voter object missing" };
    ...
    if (voter.age < (rules.minAge || 18)) {
      return {
        valid: false,
        reason: `Voter must be at least ${rules.minAge || 18}`,
      };
    }
